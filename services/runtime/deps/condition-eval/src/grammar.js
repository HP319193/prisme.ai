...
(function () {
  ...
  const ConditionalExpression =
    require('./interpreter/ConditionalExpression').default;
  const NegationExpression =
    require('./interpreter/NegationExpression').default;
  ...
  const retrieveActualString =
    (seperator = "'") =>
    ([value]) => {
      const [, group] = new RegExp(`${seperator}(.*)${seperator}`).exec(value);
      return group;
    };

  const toConditionalExpression = (items) =>
    new ConditionalExpression(items[0], items[4], items[2]);
  var grammar = {
    Lexer: lexer,
    ParserRules: [
      ...
      {
        name: 'suffixOperator',
        symbols: [lexer.has('exists') ? { type: 'exists' } : exists],
        postprocess: id,
      },
      {
        name: 'suffixOperator',
        symbols: [lexer.has('notExists') ? { type: 'notExists' } : notExists],
        postprocess: id,
      },
      {
      ...
      {
        name: 'equalityExpression$ebnf$1',
        symbols: ['equalityExpression$ebnf$1', 'insideARegEx'],
        postprocess: function arrpush(d) {
          return d[0].concat([d[1]]);
        },
      },
      {
        ...
        symbols: [
          ...
          'matchOperator',
          lexer.has('ws') ? { type: 'ws' } : ws,
          lexer.has('regex') ? { type: 'regex' } : regex,
          'equalityExpression$ebnf$1',
          lexer.has('closingP') ? { type: 'closingP' } : closingP,
        ],
        postprocess: ([left, , operator, , , right]) =>
          toConditionalExpression([left, , operator, , right]),
      },
      {
        name: 'equalityExpression',
        symbols: [
      ...
      {
        name: 'matchOperator',
        symbols: [lexer.has('matches') ? { type: 'matches' } : matches],
      },
      {
        name: 'matchOperator',
        symbols: [
          lexer.has('notMatches') ? { type: 'notMatches' } : notMatches,
        ],
      },
      {
        name: 'insideARegEx',
        symbols: [lexer.has('anything') ? { type: 'anything' } : anything],
        postprocess: ([value]) => value.value,
      },
      ...
      {
        name: 'equalityOperator',
        symbols: [lexer.has('notEquals') ? { type: 'notEquals' } : notEquals],
        postprocess: id,
      },
      {
        name: 'equalityOperator',
      ...
      {
        name: 'unaryExpression',
        symbols: ['undefinedLiteral'],
        postprocess: ([value]) => new Variable(value),
      },
      {
        name: 'unaryExpression',
        symbols: ['number'],
      ...
      },
      {
        name: 'variable',
        symbols: [
          lexer.has('dcbl') ? { type: 'dcbl' } : dcbl,
          '_',
          'variablePath',
          '_',
          lexer.has('dcbr') ? { type: 'dcbr' } : dcbr,
        ],
      ...
      },
      {
        ...
        symbols: [
          'variablePath',
          lexer.has('openingBracket')
            ? { type: 'openingBracket' }
            : openingBracket,
          'expression',
          lexer.has('closingBracket')
            ? { type: 'closingBracket' }
            : closingBracket,
        ],
        postprocess: (d) => [...arrayify(d[0]), d[2]],
      },
      {
        name: 'variablePath',
        symbols: [
          ...
          '_',
          lexer.has('word') ? { type: 'word' } : word,
        ],
        postprocess: (d) => [...arrayify(d[0]), d[4]],
      },
      {
        name: 'string',
        symbols: [lexer.has('dqstr') ? { type: 'dqstr' } : dqstr],
        postprocess: retrieveActualString(`"`),
      },
      {
        name: 'string',
        symbols: [lexer.has('sqstr') ? { type: 'sqstr' } : sqstr],
        postprocess: retrieveActualString(`'`),
      },
      {
        name: 'string',
      ...
      {
        name: 'nullLiteral',
        symbols: [lexer.has('null') ? { type: 'null' } : null],
        postprocess: () => null,
      },
      {
        name: 'undefinedLiteral',
        symbols: [lexer.has('undefined') ? { type: 'undefined' } : undefined],
