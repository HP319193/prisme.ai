...
export const AutomationBuilder: FC<AutomationBuilderProps> = ({ id, value, onChange }) => {
  ...
  const instructionsSchemas: [string, Record<string, Schema>, { icon: string }][] = useMemo(() => [
    ...
    }, {}), { icon: iconWorkspace.src }]], [automations, id, t, workspace.name])
  ...
  const addInstruction: AutomationBuilderContext['addInstruction'] = useCallback(async (parent, index) => {
    if (!parent) return;
    try {
  ...
  const removeInstruction: AutomationBuilderContext['removeInstruction'] = useCallback((parent, index) => {
  ...
  const editCondition: AutomationBuilderContext['editCondition'] = useCallback(async (parent, key) => {
    try {
      const newCondition = await editConditionDetails(key);
      const { conditions = {} as Prismeai.Conditions } = parent
      const newConditions = Array.from(new Set([...Object.keys(conditions), 'default']))
        // Sort keys with default at the end
        .sort((a, b) => b === 'default' ? -1 : 0)
        // make a map between key and value
        .map<[string, Prismeai.InstructionList]>(k => [k === key ? newCondition : k, conditions[k]])
  ...
  const getApp: AutomationBuilderContext['getApp'] = useCallback(instruction => {
    const [name, , { icon }] = instructionsSchemas.find(([, instructions]) => {
  ...
  const getSchema = useCallback((
    ...
    const schema = instructionsSchemas.reduce((prev, [, instructions]) =>
      instructions[instructionName] || prev, {} as Schema)
    return schema
  }, [instructionsSchemas])
  ...
  return (
    <automationBuilderContext.Provider value={{ addInstruction, removeInstruction, editInstruction, editCondition, editTrigger, getApp, instructionsSchemas, getSchema }}>
      <ReactFlow
        elements={elements}
      ...
      <div className="absolute bottom-0 right-0 z-5">
        <div>debug:</div>
        <div>
          <label>Nodes draggable <input type="checkbox" checked={nodesDraggable} onChange={({ target: { checked } }) => setNodesDraggable(checked)} /></label>
        </div>
      </div>
...
export const ConnectedAutomationBuilder = (props: AutomationBuilderProps) => <ReactFlowProvider><AutomationBuilder {...props} /></ReactFlowProvider>
